# Babycoin Onion Blockchain Explorer

Currently available Babycoin blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Babycoin testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Babycoin Onion Blockchain Explorer. The example not only shows how to use
Babycoin C++ libraries, but also demonstrates how to use:
...
## Babycoin Onion Blockchain Explorer features

The key features of the Babycoin Onion Blockchain Explorer are:
 ...
 - showing public components of Babycoin addresses,
 - decoding which outputs and mixins belong to the given Babycoin address and viewkey,
 - can prove that you send Babycoin to someone,
 - detailed information about ring members, such as, their age, timescale and their ring sizes,
 - showing number of amount output indices,
 - support Babycoin testnet and stagnet networks,
...
##### Compile latest Babycoin development version

Download and compile recent Babycoin into your home folder:

```bash
# first install Babycoin dependecines
sudo apt update
...
Once the Babycoin is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/babycoin
cd ~
...
# altearnatively can use: cmake -DEVOLUTION_DIR=/path/to/babycoin_folder ..
# if babycoin is not in ~/babycoin
#
...
## Enable Babycoin emission

Obtaining current Babycoin emission amount is not straight forward. Thus, by default it is
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.babycoin/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.babycoin/testnet/lmdb/emission_amount.txt` or `~/.babycoin/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.babycoin/lmdb/emission_amount.txt`
...
```
Babycoin emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Babycoin daemon's  `print_coinbase_tx_sum` command.
...
```bash
# we use here official Babycoin project's donation address as an example
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_babycoin",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "babycoin_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Babycoin.
...
Return raw json tx data, as represented in Babycoin.
